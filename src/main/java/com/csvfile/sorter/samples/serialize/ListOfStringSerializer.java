...
*
*/
public class ListOfStringSerializer implements Externalizable {
	 ...
	 */
	public ListOfStringSerializer() {
		super();
	}

	public ListOfStringSerializer( List<String> data ) {
		this.data = data;
	}
	...
	public void readExternal( ObjectInput in) throws IOException, ClassNotFoundException {
		...
		this.data = new ArrayList<String>( columnCount );
		
		for( int i = 0; i < columnCount; i++) {
			if( in.readBoolean() ) {
				this.data.add(in.readUTF());
			} else {
				this.data.add(null);
			}
		}
